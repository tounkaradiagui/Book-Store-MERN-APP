...
const EditBook = () => {
  ...
  const handleUpdateBook = () => {
    ...
    setLoading(true);
    axios.put(`http://localhost:8001/books/${id}`, data).then((res) => {
      setLoading(false);
      navigate('/');
    ...
    <div className='p-4'>
      ...
      {loading ? (
        ...
        <div className='flex flex-col border-2 border-sky-400 rounded-xl w-[600px] p-4 mx-auto'>
          ...
          <div className='my-4'>
            <label htmlFor="" className='text-xl mr-4 text-gray-500'>Auteur</label>
            <input type="text" value={author} onChange={(e) => setAuthor(e.target.value)} className='border-2 border-gray-500 px-4 py-2 w-full'/>
          </div>

          <div className='my-4'>
            <label htmlFor="" className='text-xl mr-4 text-gray-500'>Année de Publication</label>
          ...
          <button className='p-2 bg-sky-300 m-8' onClick={handleUpdateBook}>Enregistrer</button>
        </div> 
      )}
    </div>
  )
